...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (!(line = scanner.nextLine()).equals("End")) {
            ...
            Private soldier = createSoldier(line);
    ...
    private static Private createSoldier(String line) {
        Private aPrivet;
        ...
        switch (type) {
            case "Private":
                aPrivet = new PrivateImpl(Integer.parseInt(tokens[1]),
                        tokens[2],
                        tokens[3],
                        Double.parseDouble(tokens[4]));
                break;
            case "LieutenantGeneral":
                aPrivet = createLieutenantGeneral(tokens);
                break;
            case "Engineer":
                aPrivet = createEngineer(tokens);
                break;
            case "Commando":
                aPrivet = createCommando(tokens);
                break;
            case "Spy":
    ...
    private static Private createCommando(String[] tokens) {
        ...
        if (corps.equals("Airforces") || corps.equals("Marines")) {
            ...
            for (int i = 6; i < tokens.length; i += 2) {
                ...
                Mission mission = new MissionImpl(codeName, state.equals("inProgres") ? MissionImpl.Status.IN_PROGRESS
    ...
    private static Private createLieutenantGeneral(String[] tokens) {
        LieutenantGeneralImpl lieutenantGeneral = new LieutenantGeneralImpl(Integer.parseInt(tokens[1]),
                tokens[2],
                tokens[3],
                Double.parseDouble(tokens[4]));
